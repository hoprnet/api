...
export default async function handler(
  ...
  if (!process.env.FAUCET_REDIS_URL)
    return res
      .status(401)
      .json({ err: "No database to store nonces defined in server." });
  if (!process.env.FAUCET_SECRET_API_KEY)
    return res
      .status(401)
      .json({ err: "No secret api token defined in server" });
  if (secret != process.env.FAUCET_SECRET_API_KEY)
    return res.status(401).json({ err: "Secret passed is incorrect" });
  if (method != "POST")
