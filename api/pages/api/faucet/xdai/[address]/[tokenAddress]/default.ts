...
export default async function handler(
  ...
  const { method, query: { address, tokenAddress }, body: { secret } } = req;
  ...
  try {
    ...
    const tokenAddressContract = utils.getAddress(tokenAddress instanceof Array ? tokenAddress[0] : tokenAddress)
    ...
    if (err.code == errors.INVALID_ARGUMENT) return res.status(422).json({ err: 'At least one address given is incorrect' })
  }
}
